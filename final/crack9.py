...
def randomquestions(avaliblenextsquares,lastsquare,correctnextsquare,dificulty):
    randomquestionsettings={
        'additionmax':10**dificulty,
        'additionmin':1,
        'allownegativeaddition':dificulty!=1
    }     
    number1=0
    number2=0
    operations=["+","-","*","/"]
...
def maze(lastSquare,canvas,lineplace,directions,correctdirections,difficulty):
    hold=False
    mazeDone=False
    while mazeDone==False:# repeat until maze done
        ...
        try:
            ...
            correctnextsquare=0
        nextsquare=randomquestions(avaliblenextsquares,lastSquare,correctnextsquare,difficulty)# ask what move they want next then add that to directions and mark that area as having been in
        if hold==False:
...
def questions():
    ...
    return mazesizeinput,difficulty

def difficultyadvanced():
    print("ADVANCED")
...
def start():
    ...
    while True:
        directions,holddirections,holdcanvas,holdlineplace=maze(lastSquare,canvas,lineplace,directions,correctdirections,difficulty)
