...
def randomquestions(avaliblenextsquares,lastsquare):     
    dificulty=3
    number1=0
    number2=0
    operations=["+","-","*","/"]
...
def maze(lastSquare,canvas,lineplace,directions):
    ...
    while mazeDone==False:# repeat until maze done
        ...
        if len(avaliblenextsquares)==0:
            break
        nextsquare=randomquestions(avaliblenextsquares,lastSquare)# ask what move they want next then add that to directions and mark that area as having been in
        if hold==False:
